...
RUN apt-get update \
   && DEBIAN_FRONTEND=noninteractive apt-get -q -y install git \
      ...
      libreadline6 libreadline6-dev zlib1g zlib1g-dev wget \
   && apt-get -q clean \
...
RUN wget -O - https://github.com/sstephenson/rbenv-gem-rehash/archive/master.tar.gz \
   | tar zxf - \
   && mv rbenv-gem-rehash-master $HOME/.rbenv/plugins/rbenv-gem-rehash
